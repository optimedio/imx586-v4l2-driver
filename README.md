...
Special thanks to Octopuscinema and Soho-enterprise, the driver is based on their imx586 driver here:
https://github.com/octopuscinema/linux-camera-support/blob/rpi-6.1.y/drivers/media/i2c/imx586.c
...
```bash
git clone https://github.com/optimedio/imx586-v4l2-driver.git
cd imx586-v4l2-driver/
```
...
In the opened editor, locate the line containing `camera_auto_detect` and change its value to `0`. Then, add the line `dtoverlay=imx586`. So, it will look like this:
...
```
camera_auto_detect=0
dtoverlay=imx586,always-on,mono,cam0
```
